...
public class WebContactsPlugin {
    ...
    public void getContactInfo(WebView webView, Context context, Cursor currentCursor) throws JSONException {
        if(currentCursor.moveToFirst()){
            ...
            String jsUrl = "boncAppEngine.chooseContact.chooseContactSuccessHandler(" + jsonObject.toString() + ")";
            webView.loadUrl("javascript:"+jsUrl);
        }else{
            JSONObject errorJson = new JSONObject();
            errorJson.put("description","选择联系人失败");
            String jsUrl = "boncAppEngine.chooseContact.chooseContactErrorHandler(" + errorJson.toString() + ")";
            webView.loadUrl("javascript:"+jsUrl);
        }
    }
    ...
    public void cancelGetConcact(WebView webView ){
        ...
        try {
            json.put("description","取消选择联系人");
        } catch (JSONException e) {
            e.printStackTrace();
        }
        String jsUrl = "boncAppEngine.chooseContact.chooseContactCancleHandler("+json.toString()+")";
